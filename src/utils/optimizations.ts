...
import type { ImageLayout, ImageProps } from "~/types"
...
const getStyle = (args: ImageStyleProps) => {
  const { width, height, aspectRatio, layout, objectFit = "cover", objectPosition = "center", background } = args
...
interface ImageBreakpointsProps {
  width?: number
  breakpoints?: number[]
  layout: ImageLayout
}

const getBreakpoints = (args: ImageBreakpointsProps): number[] => {
  const { width, breakpoints, layout } = args
