...
import { astroAssetsOptimizer, isUnpicCompatible, unpicOptimizer } from "./optimizations"
...
const load = async function () {
  ...
  try {
    ...
    // ignore - continue regardless of error
  }
  ...
  const adaptedImages = await Promise.all(
    images.map(async (image) => {
      if (image?.url) {
        ...
        let _image: ImageMetadata | string | undefined = undefined
